...
export async function loadAllPoliticians(): Promise<Politician[]> {
  ...
  return parlamentares.map((p) => ({
    ...
    // campo opcional `legislaturas` não está presente no modelo Prisma; deixamos undefined
    // para que a interface opcional seja satisfeita.
    legislaturas: undefined,
  }));
  ...
  function formatTenure(anos: number, meses: number, dias: number): string {
    const parts: string[] = [];
    if (anos) parts.push(`${anos} ${anos === 1 ? "ano" : "anos"}`);
    if (meses) parts.push(`${meses} ${meses === 1 ? "mês" : "meses"}`);
