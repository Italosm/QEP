...
export async function getAllPoliticians(): Promise<Politician[]> {
  ...
  return (
    records
      .map((r) => {
        // 1. Cria o array já filtrando as legislaturas antigas
        const legislaturas = r.historico_legislaturas
          ? r.historico_legislaturas
              .split(",")
              .map((s) => Number(s.trim()))
              .filter(Boolean)
              .filter((leg) => leg >= 48) // MARCO: Apenas a partir de 1987
          : [];
        ...
        return {
          ...
          // 2. Atualiza a quantidade para refletir apenas a fase moderna
          quantidadeLegislaturas: legislaturas.length,
        } as unknown as Politician;
      })
      // 3. Remove da lista final políticos que só atuaram antes de 1987
      .filter((p) => (p.legislaturas?.length ?? 0) > 0)
  );
}
...
export async function getPoliticianById(
  ...
  // Aplica o mesmo filtro para a página de detalhes
  ...
  // Se o político existir no banco mas não tiver mandatos pós-87, retorna nulo (404)
  if (legislaturas.length === 0) return null;

  const primeira = legislaturas[0];
  const ultima = legislaturas[legislaturas.length - 1];
  ...
  return {
    ...
    legislaturas,
    quantidadeLegislaturas: legislaturas.length, // Atualiza contagem
...
export async function getDemocracyPoliticians(): Promise<Politician[]> {
  const records = await prisma.parlamentar.findMany();
...
export async function getDemocracyPoliticianById(
  id: string,
): Promise<Politician | null> {
  const record = await prisma.parlamentar.findUnique({
    where: { id_unico: id },
